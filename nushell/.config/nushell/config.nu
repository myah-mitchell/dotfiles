...
#
# nu is an optional secondary shell here (installed via install.sh --nushell).
# zsh is the default shell zellij and bash hand off to; launch nu manually
# with `nu` whenever you want it.
...
$env.config = {
  ...
  completions: {
    ...
    partial: true       # Controls partial completion behavior.
...
# ── Table Display ─────────────────────────────────────────────────────────────
  ...
  # No plugin settings block here. The cargo-only plugins from nu's earlier
  # setup (nu_plugin_skim/dns/highlight) aren't installed by --nushell; add
  # one back under `plugins:` if that layer gets restored later.
}

# ── Catppuccin Mocha theme ────────────────────────────────────────────────────
# Sourced after $env.config above, not before: the upstream theme file works
# by mutating $env.config.color_config/.explore as a side effect of being
# sourced, rather than exporting a record this file assigns somewhere. If it
# were sourced before the $env.config = { ... } block above, that block's own
# unconditional assignment would wipe out everything the theme just set.
...
# In practice this usually no-ops: zsh is the default shell and already starts
# Zellij before nu is ever launched manually, so $env.ZELLIJ is already set.
...
# Nu has no history-expansion syntax of its own, unlike zsh's native `!!`
# (which is what `sudo !!` uses over there instead of a dedicated command).
def "sudo!!" [] {
...
# Bypasses whatever this file's aliases.nu overrides a command name with.
# zsh's builtin `command <tool>` / `\<tool>` covers this over there, but Nu
# has no equivalent escape hatch of its own.
