...
# eza (ls): Catppuccin Mocha theme via $env.EZA_CONFIG_DIR/theme.yml (env.nu).
# Shadows Nu's own structured-table ls with eza's plain text output, same
# tradeoff zsh/.config/zsh/aliases.zsh makes for its ls alias.
...
# default, unlike GNU ls -l); --header labels every column. Matches the
# zsh `l` alias exactly, since `ls` above resolves to the same eza call.
