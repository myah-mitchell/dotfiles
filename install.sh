...
# Usage: ./install.sh [--update] [--link] [--copy] [--remove] [--windows] [--cargo] [--claude] [--full] [--nushell] [--cleanup] [--check-pins] [--upgrade=<name>] [--help]
#
...
# --nushell     Install Nushell (nu) as an optional secondary shell alongside zsh, which
#               stays the default shell regardless of this flag: zellij's default_shell
#               stays "zsh" and bash's exec-hop still lands in zsh; `nu` is just available
#               to launch manually. Downloads a prebuilt nu binary and deploys the
#               nushell/ config package (only when this flag is passed; see
#               WANT_NUSHELL below), plus the Catppuccin theme and atuin/starship/zoxide/
#               carapace init scripts nu's config sources. Not implied by --full or any
#               other flag: standalone opt-in, unrelated to the IDE-workstation axis.
#               The cargo-only Nushell plugins (nu_plugin_skim/dns/highlight) from the
#               pre-zsh-migration setup are deliberately not restored yet.
...
#               declares (e.g. a tool removed from CORE_TOOLS/CLI_TOOLS, nu if this
#               run isn't --nushell, or ccstatusline if this run isn't --claude/--full;
#               see WANT_NUSHELL/WANT_CLAUDE_STACK below), and, when this run
...
${BOLD}Flags:${RESET}
  ...
  ${CYAN}--nushell${RESET}   Install Nushell (nu) as an optional secondary shell; zsh stays
              the default either way. Deploys the nushell/ config package,
              nu's Catppuccin theme, and its atuin/starship/zoxide/carapace
              init scripts. Not implied by --full.
  ${CYAN}--cleanup${RESET}   Remove stale/orphaned local state left behind by tools removed
              ...
              rustup/cargo toolchain, --full's extra Mason LSPs, and
              --nushell's nu binary
...
# Standalone opt-in, unlike WANT_CLAUDE_STACK above: --full does NOT imply
# --nushell. Nushell is an alternative secondary shell, unrelated to the
# IDE-workstation axis --full gates; a full Neovim workstation has no more
# reason to want nu installed than a minimal one does.
WANT_NUSHELL=false
[[ "$INSTALL_NUSHELL" == true ]] && WANT_NUSHELL=true
...
chmod 700 "$HOME/.ssh"
touch "$VERSIONS_FILE"
# Nushell sources secrets.nu at parse time, so the file must exist even empty
# for that to succeed, unlike zsh's guarded (runtime) `[[ -f ... ]] && source`.
# Only set up when --nushell is actually installing nu; see WANT_NUSHELL above.
if [[ "$WANT_NUSHELL" == true ]]; then
...
if [[ "$LINK_ONLY" == false ]]; then
  ...
  # cleanup_orphans()'s CORE_TOOLS-diff sees it as "known" only on runs that
  # actually want it, with no extra code needed there.
  if [[ "$WANT_NUSHELL" == true ]]; then
    CORE_TOOLS+=(
  ...
  # ── Nushell Catppuccin theme (gated behind --nushell) ───────────────────────
  # Official catppuccin/nushell port, sourced by nushell/config.nu. Only the
  # config.nu theme is fetched; the matching nu_plugin_highlight tmTheme isn't,
  # since that plugin isn't installed (see WANT_NUSHELL above).
  if [[ "$WANT_NUSHELL" == true ]]; then
...
# nushell is the one package deployed conditionally rather than always: its
# config is inert clutter on a machine that never asked for nu (unlike e.g.
# the claude package, whose settings.json is useful independent of whether
# WANT_CLAUDE_STACK downloaded the CLI). --remove is included unconditionally
# though (regardless of WANT_NUSHELL): remove_package() only ever deletes
# symlinks that actually exist, so it's a harmless no-op on a machine that
# never had --nushell deployed, and this way a plain --remove still tears
# down the nushell symlinks it left behind on a machine that did.
if [[ "$WANT_NUSHELL" == true || "$DO_REMOVE" == true ]]; then
  PACKAGES+=(nushell)
fi
...
# nu: only generated under --nushell; nushell/config.nu sources these same
# tools' init.nu files the way zsh/.zshrc sources the init.zsh ones above.
if [[ "$WANT_NUSHELL" == true ]]; then
  if [[ ! -f "$HOME/.local/share/atuin/init.nu" ]] && { command -v atuin &>/dev/null || [[ -f "$LOCAL_BIN/atuin" ]]; }; then
    ATUIN="${LOCAL_BIN}/atuin"
    [[ -f "$ATUIN" ]] || ATUIN="atuin"
    "$ATUIN" init nu --disable-up-arrow >"$HOME/.local/share/atuin/init.nu" 2>/dev/null && ok "atuin init generated (nu)"
  fi
  ...
  if [[ ! -f "$HOME/.cache/starship/init.nu" ]] && { command -v starship &>/dev/null || [[ -f "$LOCAL_BIN/starship" ]]; }; then
    ...
    [[ -f "$STARSHIP" ]] || STARSHIP="starship"
    mkdir -p "$HOME/.cache/starship"
    "$STARSHIP" init nu >"$HOME/.cache/starship/init.nu" 2>/dev/null && ok "starship init generated (nu)"
  fi
  ...
  if [[ ! -f "$HOME/.local/share/zoxide/init.nu" ]] && { command -v zoxide &>/dev/null || [[ -f "$LOCAL_BIN/zoxide" ]]; }; then
    ...
    [[ -f "$ZOXIDE" ]] || ZOXIDE="zoxide"
    mkdir -p "$HOME/.local/share/zoxide"
    "$ZOXIDE" init nushell >"$HOME/.local/share/zoxide/init.nu" 2>/dev/null && ok "zoxide init generated (nu)"
  fi

  if [[ ! -f "$HOME/.cache/carapace/init.nu" ]] && { command -v carapace &>/dev/null || [[ -f "$LOCAL_BIN/carapace" ]]; }; then
    CARAPACE="${LOCAL_BIN}/carapace"
    [[ -f "$CARAPACE" ]] || CARAPACE="carapace"
    "$CARAPACE" _carapace nushell >"$HOME/.cache/carapace/init.nu" 2>/dev/null && ok "carapace init generated (nu)"
  fi
fi
