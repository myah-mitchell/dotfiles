...
This task's session docs folder (`~/.local/.claude/<repo-name>/<task-branch-name>/`, set up in `/start-task` Step 4) lives outside the worktree entirely, so removing the worktree above has no effect on it. Leave it in place — don't delete it as part of this or any other cleanup step. It's kept as a durable record of the task, not scratch.
...
- Never delete the task's session docs folder under `~/.local/.claude/<repo-name>/<task-branch-name>/`. It sits outside the repo specifically so cleanup never touches it.
