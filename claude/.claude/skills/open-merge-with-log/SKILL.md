...
## Purpose
Same end result as `/open-merge` (a pushed branch ready for a merge request), but first brings this task's session docs into the repo so they travel with the branch and are visible to reviewers, instead of staying in the external `~/.local/.claude/<repo-name>/<worktree-name>/` folder `/start-task` writes them to.
...
cp -r "$SESSION_DIR" ".claude-session/$WORKTREE_NAME"
```
...
```
git add ".claude-session/$WORKTREE_NAME"
git commit -m "Add session log for $WORKTREE_NAME"
```
