...
## Nushell: optional secondary shell

Nushell was removed entirely in the zsh migration (see the section above) but was later restored as an **opt-in** `install.sh --nushell` flag: zsh stays the default/primary shell in every case; `--nushell` only makes `nu` available to launch manually. `WANT_NUSHELL` (computed once, right after arg parsing, the same way `WANT_CLAUDE_STACK` is) gates everything below. It is a standalone flag, not implied by `--full` or any other flag, since Nushell is unrelated to the IDE-workstation axis `--full` gates.

**What `--nushell` installs:**
- A prebuilt `nu` binary, appended to `CORE_TOOLS` conditionally (`CORE_TOOLS+=(...)` inside `if [[ "$WANT_NUSHELL" == true ]]`, after the unconditional starship/zellij rows). Appending rather than declaring the rows unconditionally means `cleanup_orphans()`'s existing `CORE_TOOLS`-diff logic (see "Config deployment: hand-rolled linker" below) automatically treats `nu` as orphaned on any `--cleanup` run that omits `--nushell`, with no dedicated cleanup code needed for it.
- The `nushell` config package, added to `PACKAGES` conditionally on the install side (`WANT_NUSHELL == true`) rather than unconditionally like the `claude` package: an undeployed `~/.config/nushell` would be inert clutter on a machine that never asked for `nu`, unlike `claude/.claude/settings.json`, which is useful independent of whether the Claude Code CLI itself is installed. `--remove` includes it unconditionally though (`WANT_NUSHELL == true || DO_REMOVE == true`), since `remove_package()` only ever deletes symlinks that already exist, so a plain `--remove` still tears down the nushell symlinks a prior `--nushell` run left behind.
- The official `catppuccin/nushell` theme, downloaded into `~/.config/nushell/themes/catppuccin_mocha.nu`, same download-to-a-`$HOME`-path pattern as the bat/eza/zsh-syntax-highlighting themes (see [Theme](#theme)).
- `init.nu` shell-init scripts for atuin/starship/zoxide/carapace, generated alongside the existing `init.zsh` ones with the same "skip if already present" guards.
- `~/.config/nushell/secrets.nu` is pre-touched empty (`WANT_NUSHELL` block in the directory-setup section), since Nu's `source` is parse-time-only and errors if the file doesn't exist, unlike zsh's guarded runtime `[[ -f ... ]] && source` in `.zshenv`.

**Package layout** (`nushell/.config/nushell/{env.nu,config.nu,autoload/aliases.nu}`) intentionally mirrors the original pre-migration shape: XDG-nested under `.config/nushell/`, not the flat `$HOME`-rooted layout `bash`/`zsh` use, since that's how Nu itself expects to find its config, and it's the shape the removed files were in before. `env.nu`'s variable set is kept in sync with the *current* `zsh/.zshenv` (PATH, XDG dirs, editor/diff/ripgrep/bat/eza/fzf/zellij/starship/pager/locale/color vars) rather than the stale pre-migration snapshot, minus two things `zsh` now owns instead: the ssh-agent bootstrap (`.zshrc`; any `nu` launched from inside a zsh session inherits `SSH_AUTH_SOCK` already) and WSL clipboard handling (fully superseded by `clip-clean.py` at the zellij/nvim layer, see the WSL2-specific section below). `autoload/aliases.nu` mirrors the same alias priority chain documented above, including the `ls`→`eza` override; `l` uses the same `eza` flags as its zsh counterpart (`-alg --header --sort=modified`) rather than Nu's own `ls -t`, which means "list in parallel," not "sort by time," the way GNU `ls -t` does.

**Theme sourcing order matters:** `catppuccin/nushell`'s theme file works by mutating `$env.config.color_config`/`.explore` as a side effect of being sourced, not by exporting a record `config.nu` assigns somewhere. Because `config.nu` also does one unconditional `$env.config = { ... }` to set everything else (history, editing, completions, keybindings, and so on), the theme must be sourced *after* that assignment, never before, or its mutations get wiped out by the replacement. `config.nu`'s own `$env.config` block accordingly carries no `color_config` key of its own.

**Deliberately not restored:** the three cargo-only Nushell plugins from the original setup (`nu_plugin_skim`, `nu_plugin_dns`, `nu_plugin_highlight`; none ship a prebuilt binary, so installing them means bootstrapping rustup/cargo). `--nushell` never triggers a cargo bootstrap. If this layer gets added back later, follow the pre-`pre-zsh-migration`-tag shape: a `NUSHELL_PLUGINS_CARGO` array installed via `cargo_install` when `HAVE_CARGO` is true, a registration step (`nu -c "plugin add ...; plugin use ..."`) after configs are linked, and a `plugins:` block back in `config.nu`'s `$env.config`.
...
- **Flags:** `--update` (force re-download/rebuild everything), `--link` (skip binary downloads, just re-link configs), `--copy` (copy instead of symlink — useful on NTFS or shared servers), `--remove` (tear down all deployed symlinks and exit, binaries untouched), `--windows` (install Windows-side integration under WSL2 — see below), `--cargo` (build Rust tools from source instead of downloading a prebuilt release — slow and disk-heavy), `--claude` (install the Claude Code stack — see below), `--full` (implies `--claude` and `--windows`; also installs `install_powershell()` and enables nvim's heavier Mason LSPs — see below), `--cleanup` (remove orphaned/stale local state — see below). The `rustup`/cargo bootstrap itself is gated on `--cargo` (not run otherwise) — before the Nushell→zsh migration it ran unconditionally because a couple of Nu plugins shipped no prebuilt binary at all and needed cargo regardless of the flag; with Nu gone, nothing needs cargo unless `--cargo` is explicitly requested.
- **`--windows`** — opt-in, like `--claude`/`--full`, and only meaningful under WSL2 in the first place (`WANT_WINDOWS_SETUP`, computed right after `IS_WSL` detection, is `false` on native Linux/macOS regardless of the flag). Gates the whole "WSL2 only" block below: installing Alacritty via winget, installing the JetBrainsMono Nerd Font on the Windows side, and creating the Alacritty config symlink. Was previously `--no-windows` (opt-out, on by default under WSL2) — flipped to match the `--claude`/`--full` opt-in pattern, since a WSL box isn't necessarily opened through Windows Alacritty at all (a dev container, a CI runner, a headless WSL instance someone SSHes into) and had no business running winget/PowerShell by default. Implied by `--full`.
- **`--claude` / `--full`** — both opt-**in** (every other flag opts something *out* of a default-on install).
  - `WANT_CLAUDE_STACK` (computed once, right after arg parsing, as `--claude || --full`) gates `install_claude()` (the Claude Code CLI itself, via the native `curl -fsSL https://claude.ai/install.sh | bash` installer — see Claude Code integration below), `install_node()`, `install_ccstatusline()`, and `install_claude_swap()` — none of the latter three are useful without Claude Code, so none of them install unless this run wants the stack.
  - `--full` implies `--claude` (a full IDE workstation still wants Claude Code) and additionally gates two things `--claude` alone does not: (1) installing PowerShell (`pwsh`, a ~190MB self-contained tarball, needed only by the `powershell_es` LSP), and (2) whether `nvim` enables its heavier Mason-managed LSP servers at all — `html`/`cssls`/`emmet_language_server`/`bashls`/`pyright` and the whole `lang.php` extra (`intelephense`/`phpactor`/`php-cs-fixer`/`phpcs`) — together ~460MB of Mason packages that only make sense on a full IDE workstation.
  - Since `--full` implies `WANT_CLAUDE_STACK`, Node is still installed whenever those LSPs need it — the "is node on PATH" nvim signal problem this created is unchanged from before: it can't serve as nvim's `--full` signal (it'd read true under `--claude` alone too) — see the marker-file mechanism in the Neovim section below. `powershell_es` doesn't need that marker, though: nothing but `--full` installs `pwsh`, so `nvim` checks `vim.fn.executable("pwsh")` directly.
  - Pass `--full` on a full Neovim IDE workstation, `--claude` alone on a machine that just wants Claude Code without the IDE extras, and leave both off on servers/shared boxes that only need the shell environment — that's the default.
- **`--nushell`**: opt-in, standalone (not implied by `--full` or anything else). `WANT_NUSHELL` gates installing `nu` and deploying the `nushell` config package; see [Nushell: optional secondary shell](#nushell-optional-secondary-shell) above for the full detail.
- **`--cleanup`** — `install.sh`'s downloaders are all overwrite-in-place (a `CORE_TOOLS`/`CLI_TOOLS` entry, a Zellij plugin, a zsh plugin all get re-downloaded onto the same fixed path), which means nothing ever notices when a tool is *removed* from those arrays entirely — its old binary just sits on disk forever with a stale `.versions` entry.
  - `cleanup_orphans()` (defined near the end of `install.sh`, run when `--cleanup` is passed) diffs `.versions` against the exact `CORE_TOOLS`/`CLI_TOOLS`/`ZELLIJ_PLUGINS`/`ZSH_PLUGINS` arrays used to install things — plus a short static list for the handful of bespoke installers that don't use those arrays (`nvim`, `yazi`, `jetbrainsmono-nf` always known; `node`/`ccstatusline` known only when `WANT_CLAUDE_STACK`; `pwsh` known only when `--full`) — and removes anything left over: `$BIN_DIR` binaries, Zellij `.wasm` files, zsh plugin dirs, and orphaned `npm install -g` packages (checked against `~/.local/lib/node_modules/<key>` — this is what actually uninstalls `ccstatusline` when it's orphaned).
  - **`node` is the one deliberate exception**: when orphaned it's left in place with a warning rather than auto-removed, because `install_node()`'s `cp -a` merges node's own `bin`/`lib`/`include`/`share` trees straight into the same `~/.local/{bin,lib,...}` directories that real dotfiles-deployed symlinks and other npm globals live in — unlike `pwsh`'s isolated `~/.local/lib/powershell`, there's no single path that's safely `rm -rf`-able as "node and nothing else," so guessing wrong risks deleting files this script doesn't own.
  - It also removes `~/.rustup`/`~/.cargo` outright when the run isn't `--cargo` (regenerated on demand by the next `--cargo` run), and — when the run isn't `--full` — uninstalls the fixed list of full-only Mason LSP packages (same list as the `--full` bullet above) via a headless `nvim --headless -c "MasonUninstall ..."` if any are present, since Neovim's Mason downloads aren't installed by `install.sh` at all and so can't be caught by the `.versions` diff.
  - No effect combined with `--link` (those arrays are never populated without a real run — `install.sh` warns and skips rather than risk misjudging orphans off an empty set).
  - Separately (not part of the `.versions` diff, since `install.sh`'s own tracking doesn't cover `claude` — see below), `cleanup_orphans()` also removes old Claude Code version binaries, *regardless* of `WANT_CLAUDE_STACK`: the native installer/self-updater drops each new version into `~/.local/share/claude/versions/<version>` (~300-350MB apiece) and repoints the `~/.local/bin/claude` symlink at it, but never deletes the old ones — `cleanup_orphans()` resolves that symlink and removes every sibling in `versions/` that isn't the current target, whether or not this run also passed `--claude`.
  - **When adding a new bespoke installer that doesn't go through `CORE_TOOLS`/`CLI_TOOLS`/`ZELLIJ_PLUGINS`/`ZSH_PLUGINS`, add its key to `cleanup_orphans()`'s static `known_dest` list too** — this was tested against live orphaned state on the maintainer's machine before landing, and testing caught exactly this gap once already (`yazi` uses `download_release()` directly, bypassing `CLI_TOOLS`, and got wrongly deleted as a false orphan in the first pass — fixed by adding it to the static list; `ya`, yazi's companion binary, was confirmed to never get a `.versions` entry at all, so it needed no such fix).
- **`homepath` git filter** (registered right before the `PACKAGES` linking loop) — `zellij/.config/zellij/config.kdl`'s zjstatus `command_weather_command`/`command_sysinfo_command`/`command_battery_command`/`copy_command` need real absolute paths to the scripts in `zellij/.config/zellij/scripts/`.
  - **Why:** zjstatus's WASM plugin execs commands directly (confirmed in its `src/widgets/command.rs` — the config string is split into argv and handed straight to `run_command`, no shell, no `~`/`$HOME` expansion of any kind) and this has nothing to do with which shell the user runs interactively, since zjstatus never invokes one either way. `config.kdl` is symlinked (not copied) straight into `$HOME`, so a naive absolute path would carry whichever machine's username last edited it.
  - **How:** the file in git stores a `__DOTFILES_HOME__` placeholder, and `.gitattributes` marks it `filter=homepath`: `git-filters/homepath-smudge.sh` expands the placeholder to the real `$HOME` on checkout (so the deployed file works), and `git-filters/homepath-clean.sh` folds it back to the placeholder before anything is staged (so `git add`/`git commit` from any machine never stages a machine-specific path — no more manually excluding those hunks when committing from a different box).
  - Filter *commands* are local git config, not versioned, so `install.sh` re-registers them (`git config filter.homepath.*`) and force-checks-out the file on every run, in case the filter wasn't registered yet the last time it was written to the working tree (e.g. right after a fresh clone).
  - Two subtle failure modes bit this in practice, so `install.sh` now guards both: (1) `git-filters/homepath-*.sh` must themselves stay executable in git (git execs them directly, no `sh -c` wrapper — same pitfall as the five scripts below), so `install.sh` `chmod +x`s them defensively before registering the filter; (2) `git checkout-index -f` only forces *overwriting*, it does not bypass git's stat-based freshness check, so if the working-tree file's mtime already matches the index (e.g. it was checked out once before the filter existed) it silently skips re-smudging with no error — `install.sh` now `rm`s the file immediately before the `checkout-index -f` call to force a real rewrite, then greps for a leftover `__DOTFILES_HOME__` and fails loudly if the filter didn't actually run.
  - **Neovim's equivalent problem doesn't need this filter**: `nvim/.config/nvim/lua/config/autocmds.lua`'s WSL clipboard `copy` entry has the exact same "no shell expansion" constraint (confirmed the same way), but unlike static KDL data, that file is Lua — evaluated by nvim itself on whatever machine it's running on — so it just computes the path with `vim.fn.expand("~/.config/zellij/scripts/clip-clean.py")` at load time instead of baking one in. No placeholder, no filter, no `install.sh` involvement needed for that file at all. If a future file has this same problem, check whether it's *executable config* (can compute its own path — do that) before reaching for this filter (only for genuinely static data a non-shell-invoking consumer reads as-is).
  - **Must run before the `PACKAGES` linking loop, not after** — this bit a real ansible-managed deploy (that role deliberately excludes `zellij/.config/zellij/config.kdl` from whatever puts the tree on the target host, since it's meant to always be regenerated by this block instead; see its `sync.yml` for the exact rationale). On a host's first-ever `install.sh` run, that exclusion means this `git checkout-index` is the *only* thing that ever creates `config.kdl` in the repo checkout at all. `link_package` only symlinks files that already exist when it walks a package directory (`find "$pkg_dir" -type f`) — with the filter block placed after the `PACKAGES` loop, a first-ever run had no `config.kdl` in the checkout yet when `link_package("zellij")` looked for it, so it silently skipped that one file (no warning) and `~/.config/zellij/config.kdl` was simply never linked. Every other file in the `zellij` package is present in the repo tree normally, so nothing else was affected — only this one file, and only on a host's very first run.
...
**Run log:** every invocation appends its full output to `~/.local/state/dotfiles/install.log`, set up right after arg parsing (before the standalone early-exit flows like `--check-pins`/`--upgrade`/`--yubikey`/`--remove`, so those get logged too, not just the main install path). This exists because `.versions` and `full-install` (below) are current-state snapshots that get overwritten each run — neither is a history of what actually happened on past runs. Each run is bracketed by a timestamped `=== ... ===` header/footer line (the footer includes the exit status); colors are stripped before writing to the file via a `sed` filter in the `tee` pipeline (terminal output stays colored) since ANSI codes are just noise once the file is opened outside a terminal. Trimmed to the most recent 5000 lines at the start of each run — no separate rotation step, and not covered by `--cleanup` (it's a log, not a stale/orphaned tool install).
...
- **Language support:** enabled via LazyVim `lang.*` extras where they exist.
  - `lang.rust` (rust-analyzer, no Node dependency, always on).
  - `lang.python` (pinned to **pyright** via `vim.g.lazyvim_python_lsp` in `options.lua`, *not* basedpyright — basedpyright installs via pip and there is none here).
  - `lang.php` (intelephense, a pure-Node analyzer, so no PHP runtime needed).
  - `lang.markdown` (marksman, markdownlint-cli2, markdown-toc, prettier — see the markdownlint defaults note below).
  - HTML/CSS/Emmet/Bash have no lang extras, so their servers are declared in `plugins/lsp-servers.lua`.
  - Those, plus `pyright` and the whole `lang.php` extra, are all Node/npm packages installed by Mason — which is why `install.sh` installs a Linux **Node.js into `~/.local`** when this run wants the Claude Code stack (`--claude`/`--full` — see the Flags section above; `--full` always wants it for exactly these LSPs) (the Windows `npm` leaking onto PATH via WSL interop would install unusable Windows binaries; `zsh/.zshenv` prepends `~/.local/bin` so the Linux one wins).
  - **ruff** is the exception — pip-based in Mason — so `install.sh` installs it as a standalone binary and `plugins/lsp-servers.lua` sets `ruff = { mason = false }` so Mason doesn't try; ruff is always on, no Node/pwsh dependency.
  - Treesitter parsers for all seven languages live in `plugins/treesitter.lua`, so highlighting works even where an LSP can't be installed.
  - **PowerShell** (`powershell_es`) runs on `pwsh`, which `install.sh --full` installs as a self-contained tarball into `~/.local/lib/powershell` (symlinked to `~/.local/bin/pwsh`) — it bundles its own .NET runtime, so no separate .NET/root is needed; Mason supplies the PowerShell Editor Services bundle.
- **markdownlint defaults:** `lang.markdown`'s `markdownlint-cli2` linter (via `nvim-lint`) flags MD013 (line-length) and MD041 (first line must be a top-level heading) by default — both too noisy for prose/READMEs to want on unconditionally. `plugins/lint.lua` overrides `mfussenegger/nvim-lint`'s `markdownlint-cli2` linter args (LazyVim's own `opts.linters` extension point — see `lazyvim.plugins.linting`) to add `--config <nvim config dir>/.markdownlint.yaml`, a fixed file (`nvim/.config/nvim/.markdownlint.yaml`) that turns both rules off. `--config` sets markdownlint-cli2's *base* config rather than replacing discovery outright (confirmed empirically, not just from the `--help` text) — a project's own `.markdownlint.yaml`/`.markdownlint-cli2.yaml` still layers on top and can re-enable either rule for that project. The path is computed via `vim.fn.stdpath("config")` at load time rather than hardcoded, the same "Lua can compute its own path" reasoning as the homepath git filter note above — no filter needed here since this file only ever needs *this* machine's config dir, never a baked-in one.
- **`--full`-gated LSPs (Mason disk footprint):** `html`/`cssls`/`emmet_language_server`/`bashls`/`pyright` and the `lang.php` extra are ~460MB combined once Mason downloads them, so they're only enabled on a full IDE workstation.
  - `config/lazy.lua` reads `~/.local/state/dotfiles/full-install` (a marker file `install.sh` writes/removes every run based on `--full`) to decide whether to `table.insert` the `lang.php` extra into its plugin spec at all; `plugins/lsp-servers.lua` reads the same marker to decide whether to declare `html`/`cssls`/`emmet_language_server`/`bashls`, and sets `servers.pyright = false` outside `--full` to override `lang.python`'s own unconditional pyright config (LazyVim's documented idiom for opting a server out of Mason auto-install — `false` gets normalized to `{ enabled = false }` in `lazyvim.plugins.lsp.init`, which both skips `vim.lsp.enable()` and excludes it from `mason-lspconfig`'s `ensure_installed`).
  - The marker exists because Node.js is gated behind `WANT_CLAUDE_STACK` (`--claude` *or* `--full`, not `--full` alone — see above), so nvim can't just check `vim.fn.executable("node")` the way it checks `vim.fn.executable("pwsh")` for `powershell_es`: that would read true under `--claude` alone too, without `--full` having been passed.
  - Toggling `--full` on install.sh reconfigures what nvim *would* enable on next launch, but does **not** retroactively uninstall whatever Mason already downloaded under the old setting — that's a manual `:Mason` / `:MasonUninstall <package>` cleanup step.
  - `lang.markdown`'s Node tooling (marksman/markdownlint-cli2/markdown-toc/prettier, ~30MB) is deliberately left ungated — small enough, and common enough to want on any machine, not to bother tying to `--full`.
...
**`claude` package** (in `PACKAGES`, unconditional — always deployed regardless of `WANT_CLAUDE_STACK`, since it's just a config symlink, not a download): symlinks `~/.claude/settings.json` from `claude/.claude/settings.json`, so the Notification hook below (and `theme`) are version-controlled across machines. Also symlinks `~/.claude/keybindings.json` from `claude/.claude/keybindings.json` — currently just remaps `alt+enter`/`shift+enter`/`ctrl+enter` to `chat:newline`; only `alt+enter` actually fires over this repo's WSL2 + Windows Terminal + Zellij stack (Shift+Enter and Ctrl+Enter both collapse to bytes already claimed by something else — see the git history around this file's addition for the full breakdown), the other two are harmless no-ops kept in case the terminal side ever starts disambiguating them. `claude/.claude/CLAUDE.md` — a generic pair-programming prompt left over from the initial repo import, never reviewed as real global instructions — is deliberately excluded from deployment via `claude/.linkignore`; delete that line once it's been reviewed and you actually want it live.
...
**Local, on-demand complement:** `./install.sh --check-pins` prints the same three groups (`ZELLIJ_PLUGINS`, `ZSH_PLUGINS`, `ccstatusline`) next to their current upstream version and exits — read-only, no downloads. `./install.sh --upgrade=<name>` (e.g. `--upgrade=zjstatus.wasm`, `--upgrade=ccstatusline`) bumps exactly *one* of them: rewrites that pin in `install.sh` via `sed` (validating the fetched tag/version against `^[A-Za-z0-9._-]+$` first, since it's untrusted upstream input being spliced into the script's own source — sed has no `%ENV`-style passthrough the way the CI script's Perl does, so this validates instead), installs it immediately, and prints the same manual-review note a bump PR for it would carry — then exits, leaving the change unstaged for the user to review and commit. Deliberately one dependency per invocation, not a blanket "upgrade everything," for the same reason each CI bump PR is reviewed individually. Both flags share their lookup logic with the CI script's `github-release`/`npm` kinds, plus a `github-tag` lookup for `ZSH_PLUGINS` (the kind the CI checker still lacks — see above) that prefers clean `vX.Y.Z`-style tags over ones with a pre-release suffix, since a plain `sort -V` over the raw tag list picks a pre-release tag as "newest" on at least one real repo in this array (confirmed against `zsh-users/zsh-syntax-highlighting`, where `0.8.0-alpha1-pre-redrawhook` sorts above `0.8.0` despite being the *older* tag). Both flags run before the zsh/git/locale prerequisite checks further down `install.sh`, so a version check never triggers a `sudo` git/locale install as a side effect.

---

## SSH: YubiKey resident key retrieval

`./install.sh --yubikey` retrieves SSH key(s) already provisioned as **FIDO2 resident keys** on a YubiKey (or any FIDO2 authenticator) via `ssh-keygen -K`, which downloads them straight into `~/.ssh/`. This is retrieve-only — it does not provision a new resident key on the YubiKey itself; run `ssh-keygen -t ed25519-sk -O resident` first (on this machine or another) if the YubiKey doesn't have one yet. Standalone like `--check-pins`/`--upgrade`/`--remove` above: does its own thing and exits, no interaction with the rest of the install flow.

Three deliberate choices worth knowing if this needs touching again:

- **FIDO2 resident keys, not PIV/PKCS#11 or GPG.** `ssh-keygen -K` was picked over a PKCS#11 provider (`ykcs11.so`, key stays on the device, used live via `IdentityFile`+`PKCS11Provider`) or gpg-agent's SSH support (for a YubiKey provisioned with GPG auth subkeys) because it's the standard "materialize this identity on a new machine" flow for OpenSSH 8.2+, with the smallest dependency footprint (just an OpenSSH client built with security-key support, no yubico-piv-tool/pcscd/gpg-agent stack).
- **No apt install for the dependency.** `ssh-keygen -K` needs an OpenSSH client with FIDO2 support at runtime, but rather than preflight-detecting that (fragile — package names, whether the distro links libfido2 in directly vs. dlopens it as an external "provider," and provider search paths all vary by distro/OS build), `install_yubikey`'s equivalent (`pull_yubikey_ssh_key()` in `install.sh`) just runs the real operation and classifies the failure from its output if there is one — matching the `--check`-then-error pattern of the existing zsh-presence check, not a new silent apt install (see the Deliberate Omissions table above: only mosh and git get that exception).
- **Streamed output, not captured-and-replayed.** Retrieval is interactive — the authenticator needs a physical touch and may prompt for its PIN, both of which the user needs to see and respond to in real time — so `ssh-keygen -K`'s output goes straight to the terminal via `tee` rather than being captured into a variable and printed after the fact (which would also delay/hide the "touch now" prompt until too late). A copy is kept via `tee` purely so a *failure* can still be pattern-matched afterward for the "which error was this" classification. Because `install.sh` runs under `set -eo pipefail`, this pipeline has to be the condition of an `if` (one of `set -e`'s documented exemptions) rather than a bare statement — a real, expected failure (no key plugged in yet) would otherwise abort the whole script before the classification code ever ran; this was caught by actually testing the no-device case against a real OpenSSH client rather than assuming the error path worked. That same live test is also what surfaced the classification-ordering gotcha below.
- **Classification order matters.** A real OpenSSH 10.x client (FIDO2 support linked in, not an external provider) prints `Provider "internal" returned failure -4` / `Unable to load resident keys: device not found` for the ordinary "no key plugged in" case — so a broad `provider` substring match for "libfido2 is missing" has to be checked *after* the specific "device not found" pattern, or the common case gets misdiagnosed as a missing-library problem it isn't.

`ssh-keygen -K` names its own output files based on what's stored on the resident key (not something this flag controls), so it downloads into `~/.ssh/` directly rather than a staging directory. The checked-in `ssh/.ssh/config` default (`IdentityFile ~/.ssh/id_ed25519` / `id_rsa`) doesn't know about whatever filename comes back — the flag prints a reminder to add an `IdentityFile` line by hand rather than guessing a filename to inject into the committed config.

**Under WSL2, `--yubikey` needs USB passthrough set up first — this is a Windows-side prerequisite, not something `install.sh` can arrange for you.** WSL2 is a real VM; a YubiKey plugged into the machine is invisible to its kernel until bridged over. Deliberately **not** automated into `install.sh`'s `--windows` flow (unlike Alacritty/fonts/the config symlink) — the actual bind/attach step needs the physical key plugged in, reads output only a human can act on (the device's `BUSID`), and needs a one-time interactive Windows admin elevation, so there's nothing here that's safe to script blindly. One-time manual setup:

1. Windows side: `winget install usbipd` (once), then `usbipd list` with the YubiKey plugged in to find its `BUSID`.
2. Windows side, one-time per device: `usbipd bind --busid <busid>` (needs admin).
3. Windows side, each session the key needs to be visible in WSL2 (or set up `usbipd attach --wsl --busid <busid> --auto-attach` to reattach automatically on reconnect — doesn't survive a reboot without extra Windows-side scheduling): `usbipd attach --wsl --busid <busid>`.
4. Inside WSL2: `lsusb` should now show the Yubico device (confirmed working this way on stock Ubuntu-on-WSL2, no custom kernel, per [Daniel Nashed's writeup](https://blog.nashcom.de/nashcomblog.nsf/dx/using-fido2-yubikey-on-wsl-for-ssh.htm)) — `./install.sh --yubikey` (or `ssh-keygen -t ed25519-sk -O resident` to provision a new resident key, or ordinary `ssh`/`git` using an sk- key) then work exactly as on native Linux, no code changes needed.

Two gotchas seen in the wild, not universal — try without them first:
- If `/dev/hidraw*` ends up root-owned, `ssh-keygen` needs `sudo` until a udev rule grants non-root access, e.g. `KERNEL=="hidraw*", SUBSYSTEM=="hidraw", TAG+="uaccess", MODE="0666"` (per [this FIDO2-on-WSL2 writeup](https://lgug2z.com/articles/yubikey-passthrough-on-wsl2-with-full-fido2-support/), which also had to build a custom WSL2 kernel with `HIDRAW`/`HIDDEV` enabled — Nashed's stock-kernel setup above didn't need that, so try the stock kernel first).
- A FIDO2 PIN must be set on the YubiKey before `-O resident` will work — it isn't set by default (same source).
