...
- **Flags:** `--update` (force re-download/rebuild everything), `--link` (skip binary downloads, just re-link configs), `--copy` (copy instead of symlink — useful on NTFS or shared servers), `--remove` (tear down all deployed symlinks and exit, binaries untouched), `--no-windows` (skip the WSL2 PowerShell steps), `--cargo` (force *every* Rust tool to build from source instead of downloading a prebuilt release — slow and disk-heavy). Note this is independent of the Rust toolchain itself: `install.sh` bootstraps `rustup`/cargo unconditionally (not gated on `--cargo`), because a couple of Nushell plugins (`nu_plugin_skim`, `nu_plugin_dns`, `nu_plugin_highlight` — see `NUSHELL_PLUGINS_CARGO`) ship no prebuilt binary at all and can only be installed by compiling from source; that toolchain bootstrap is non-fatal on failure (just skips those plugins) rather than aborting the run. `--cargo` only changes whether everything *else* (nu, starship, zellij, bat, fd, ...) also compiles from source instead of using its normal prebuilt-release path.
